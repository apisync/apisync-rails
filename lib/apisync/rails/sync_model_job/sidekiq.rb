        ...
        def perform(model_name, id, attributes, attempt = 1)
          ...
          begin
            Apisync::Rails::Http.post(
              ...
              too_many_requests_attempts: attempt
            )
          ...
          # this algorithm will push this job to be retried in the future.
            ...
            ::Rails.logger.warn "Too many simultaneous HTTP requests. Requests are being automatically throttled to solve this problem. Contact ApiSync support for details."
            retry_in = Random.new.rand(270) + 30 # 30 seconds - 5 minutes
            self.class.perform_in(retry_in, model_name, id, attributes, attempt + 1)
          end
        end
      end
    end
